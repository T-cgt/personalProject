  ...
  initPosition() {
    ...
    if(!window.sessionStorage.getItem('nations')){
      this.getRequest('/system/basic/employee/nation').then(resp=>{
        if(resp){
          this.nations=resp;
          window.sessionStroage.setItem('nations',JSON.stringify(resp))
        }
      })
    }
    else{
      this.nations=JSON.parse(window.sessionStroage.getItem('nations'))
    }
    if(!window.sessionStorage.getItem('joblevels')){
      this.getRequest('/system/basic/employee/joblevel').then(resp=>{
        if(resp){
          this.joblevels=resp;
          window.sessionStroage.setItem('joblevels',JSON.stringify(resp))
        }
      })
    }
    else{
      this.joblevels=JSON.parse(window.sessionStroage.getItem('joblevels'))
    }
    if(!window.sessionStorage.getItem('politicsstatus')){
      this.getRequest('/system/basic/employee/politicsStatus').then(resp=>{
        if(resp){
          this.politicsstatus=resp;
          window.sessionStroage.setItem('politicsstatus',JSON.stringify(resp))
        }
      })
    }
    else{
      this.politicsstatus=JSON.parse(window.sessionStroage.getItem('politicsstatus'))
    }
  },
    showAddEmpView(){
    ...
    initEmps(){
      this.loading=true
      this.getRequest('/system/basic/employee/?currentPage='
